...
<details>
<summary>Datatypes — number, string, boolean</summary>
...
|---|---|---|
| `number` | Getal | `42`, `3.14` |
| `string` | Tekst (tussen quotes) | `'hallo'`, `"Anna"` |
| `boolean` | Ja/nee | `true`, `false` |

```js
let leeftijd = 16;        // number
let naam = 'Anna';        // string
let isJarig = true;       // boolean
```

</details>

<details>
<summary>Rekenoperatoren — <code>+</code>, <code>-</code>, <code>*</code>, <code>/</code>, <code>%</code></summary>

| Operator | Wat | Voorbeeld |
|---|---|---|
| `+` | Optellen / strings plakken | `3 + 4` → `7`; `'Hi ' + naam` |
| `-` | Aftrekken | `10 - 3` → `7` |
| `*` | Vermenigvuldigen | `3 * 4` → `12` |
| `/` | Delen | `10 / 2` → `5` |
| `%` | Rest na deling | `10 % 3` → `1` |

Let op: `'3' + 4` is `'34'` (string-concatenatie), `3 + 4` is `7` (optellen). Gebruik `Number(x)` om een string naar een getal te zetten.

</details>

<details>
<summary>Vergelijkingsoperatoren — <code>===</code>, <code>!==</code>, <code>&gt;</code>, <code>&lt;</code></summary>

Resultaat van een vergelijking is altijd een **boolean** (`true` of `false`):
...
| `<=` | Kleiner of gelijk | `5 <= 4` → `false` |

Gebruik altijd `===` (drie tekens), niet `==`. `5 == '5'` is `true` maar `5 === '5'` is `false` — en dat laatste klopt: een getal is geen string.
...
<details>
<summary>if, else, else if</summary>

Met `if` voer je code alleen uit als een voorwaarde waar is. Met `else` doe je iets *anders* als de voorwaarde niet klopt. Met `else if` ketens van voorwaarden:
...
Combineer voorwaarden met `&&` (EN), `||` (OF), `!` (NIET):
...
**Veelgemaakte fout**: `if (score = 10)` — één `=` is toekennen, geen vergelijking. Gebruik `===`.
...
<body>
  <p id="uitvoer">Klik op de knop.</p>
  <button onclick="check()">Check cijfer</button>
</body>
</html>`}
  initialCss={`p { font-size: 1.3rem; padding: 12px; }
button { padding: 8px 16px; font-size: 1rem; cursor: pointer; }`}
  ...
  if (cijfer >= 8) {
    ...
    document.getElementById('uitvoer').textContent = 'Voldoende';
  } else {
    document.getElementById('uitvoer').textContent = 'Onvoldoende';
  }
}`}
  height="460px"
  livePreview={false}
