---
title: Les 7 – Waarden en operatoren
sidebar_label: 7. Waarden en operatoren
hide_table_of_contents: true
---

import CodeEditor from '@site/src/components/CodeEditor';

# Les 7 – Waarden en operatoren
...
**Opdracht:** Bouw een "leeftijd-checker". Sla een leeftijd op in een variabele. Toon op de pagina:
- de leeftijd zelf
- of de persoon meerderjarig is (`true` / `false`)
- een zin: "Je bent meerderjarig" of "Je bent minderjarig" (kijk hieronder bij Tip hoe — `if`/`else` komt in les 8)

<details>
<summary>Tip</summary>

Sla het resultaat van de vergelijking op in een boolean: `const meerderjarig = leeftijd >= 18;`. Voor de zin kun je dit gebruiken (een **ternary**):

```js
const zin = meerderjarig ? 'Je bent meerderjarig' : 'Je bent minderjarig';
```

De volledige `if`/`else` syntax leer je in [les 8](/docs/js-basics/if-else).
...
function check() {
  ...
  document.getElementById('zin').textContent = zin;
}
```
...
[← Les 6 – Events](/docs/js-basics/events) | [Les 8 – Beslissingen →](/docs/js-basics/if-else)
